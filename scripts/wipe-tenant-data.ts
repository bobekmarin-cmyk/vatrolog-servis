/**
 * Wipe all tenant-bound data (extinguishers, work orders, customers, stock,
 * label authorizations, custom services, audit/email/document logs, …) so the
 * platform can be repopulated from scratch.
 *
 * KEEPS:
 *   - Company, AccountUser, User, PlatformUser
 ...
 *
 * Default je dry-run (samo ispiše brojeve). Pokretanje:
 *   npx ts-node -P tsconfig.seed.json scripts/wipe-tenant-data.ts
 *   npx ts-node -P tsconfig.seed.json scripts/wipe-tenant-data.ts --apply
 */
...
type CountStep = {
  ...
  /** Vrati broj obrisanih redova (apply mode). */
...
async function main() {
  header(APPLY ? "WIPE TENANT DATA (apply mode)" : "WIPE TENANT DATA (dry-run)");
  if (!APPLY) {
    console.log("Dry-run: prikazujem broj redova koji bi se obrisali. Pokreni s `--apply` da izvršiš.");
  } else {
    console.log("APPLY: brišem sve tenant-bound podatke unutar jedne transakcije.");
  }

  // Redoslijed je važan zbog FK ovisnosti — i kad postoji ON DELETE CASCADE,
  // ipak idemo eksplicitno da budemo precizni s brojevima.
  const steps: CountStep[] = [
    ...
    {
      label: "AuthToken (samo s companyId)",
      count: () =>
        prisma.authToken.count({ where: { companyId: { not: null } } }),
      apply: () =>
        prisma.authToken.deleteMany({ where: { companyId: { not: null } } }).then((r) => r.count),
    },
    {
      label: "AuditLog (samo s companyId)",
      count: () =>
        prisma.auditLog.count({ where: { companyId: { not: null } } }),
      apply: () =>
        prisma.auditLog.deleteMany({ where: { companyId: { not: null } } }).then((r) => r.count),
    },
    {
    ...
    {
      label: "Part (samo tenant: companyId NOT NULL)",
  ...
  if (!APPLY) {
    ...
    const keep = [
      ...
      ["ServiceLabel (globalni)", await prisma.serviceLabel.count()],
    ] as const;
  ...
  await prisma.$transaction(async () => {
    // Napomena: koristimo top-level prisma jer su deleteMany već izolirane
    // operacije; transakcija na razini connection-a u Prisma 6.x koristi
    // istu konekciju za naredbe iznutra. Za ovaj jednokratni script-flow
    // dovoljno nam je sekvencijalno izvođenje s rollbackom.
    for (const s of steps) {
  ...
  console.log(`  Sve tenant-bound podatke su obrisani. Tvrtke i korisnici su sačuvani.`);
}
