...
export type RenderDeliveryNotePdfParams = {
  /** Službeni broj otpremnice (npr. 10-260001). */
  docId: string;
  /** Datum ispisan u PDF-u kao „datum otpremnice“. */
  deliveryNoteAt: Date;
  /** Trenutak za „generirano“ u podnožju. */
  generatedAt: Date;
};
...
export async function renderDeliveryNotePdfBuffer(
  ...
  companyId: string,
  workOrderId: string,
  params: RenderDeliveryNotePdfParams,
): Promise<Buffer> {
  ...
  if (order.status !== "LOCKED") {
    throw new Error("WORK_ORDER_NOT_LOCKED");
  }

  const { docId, deliveryNoteAt, generatedAt } = params;
  ...
  for (const c of [...customByName.values()].sort((a, b) => a.name.localeCompare(b.name, "hr"))) {
    services.push({
  ...
  const partsAggregated: DeliveryNotePartAggregated[] = [
    ...
    ...realItems
      .filter((i) => (i.partsText ?? "").trim().length > 0 && (i.parts ?? []).length === 0)
      .map((i) => ({
        code: "—",
  ...
  return await renderPdfToBuffer(element);
}
