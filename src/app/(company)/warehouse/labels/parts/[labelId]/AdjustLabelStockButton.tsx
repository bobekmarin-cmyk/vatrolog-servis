...
export default function AdjustLabelStockButton({
  serviceLabelId,
}: {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/warehouse/labels/adjustments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ serviceLabelId, delta: n, reason: reason.trim() }),
      });
      if (!res.ok) {
  ...
  return (
    <>
      ...
      <Modal
        open={open}
        title="Korekcija stanja naljepnica"
        variant="neutral"
        size="md"
        onClose={close}
        footer={
        ...
        <div className="space-y-3 p-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="npr. popis stanja, oštećene naljepnice, ispravak unosa…"
